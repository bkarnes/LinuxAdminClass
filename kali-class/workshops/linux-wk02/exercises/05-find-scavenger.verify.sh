#!/bin/bash
# Verify: ~/found-seed.txt contains THIS student's seed-value.
...
if [ ! -f "$HOME/found-seed.txt" ]; then
    echo "~/found-seed.txt doesn't exist yet. Find the seed file and copy it there." >&2
    exit 1
fi

if ! grep -qF "seed-value=$expected" "$HOME/found-seed.txt"; then
...
if ! grep -qF "seed-label=scavenger" "$HOME/found-seed.txt"; then
